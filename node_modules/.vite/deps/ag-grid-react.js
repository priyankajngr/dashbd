...
import { Ar as _findEnterpriseCoreModule, At as GridBodyCtrl, Di as _getRowContainerOptions, Ei as _getRowContainerClass, Ft as HeaderRowContainerCtrl, Mt as GridCtrl, Na as _processOnChange, Nt as GridHeaderCtrl, Os as AgPromise, P as BaseComponentWrapper, Pi as _getRowSpanContainerClass, Ts as updatePinnedSectionWidths, Vn as VanillaFrameworkOverrides, Yi as _isClientSideRowModel, ca as _isHeaderFocusSuppressed, da as _isMultiRowSelection, dn as RowContainerCtrl, en as ProvidedFilter, ht as EmptyBean, io as _warnWithoutAttribution, is as partitionByPinned, jn as TabGuardCtrl, jt as GridCoreCreator, li as _getGridOption, lr as _combineAttributesAndGridOptions, no as _warnForGrid, oo as applyHeaderWrapperHidden, qi as _isCellSelectionEnabled, so as applyHeaderWrapperMaxHeight, ui as _getGridRegisteredModules, va as _isServerSideRowModel, vt as FakeHScrollComp, yt as FakeVScrollComp, z as BeanStub } from "./main.esm-8LykaKZm.js";
...
new RegExp(`^\\d{4}-\\d{2}-\\d{2}([${DATE_TIME_SEPARATOR} ]\\d{2}:\\d{2}(:\\d{2})?(\\.\\d+)?(Z|[+-]\\d{2}(:?\\d{2})?)?)?$`);
...
var reactVersion = "19.3.0".split(".")[0];
...
var ReactComponent = class {
	...
	createParentElement(params) {
		...
		if (this.componentType.requiresBlockWrapper) eParentElement.classList.add("ag-react-wrapper-block");
...
function updateFloatingFilterParent(params, model) {
	params.parentFilterInstance((instance) => {
		((instance instanceof ProvidedFilter ? instance.setModel(model, true) : instance.setModel(model)) || AgPromise.resolve()).then(() => {
...
var ToolPanelComponentWrapper = class extends CustomComponentWrapper {
	...
	*/
	applyInitialState(params) {
		if (params.initialState !== void 0 && params !== this.appliedParams) this.state = params.initialState;
...
var HeaderCellComp = ({ ctrl }) => {
	...
	const setRef2 = (0, import_react.useCallback)((eRef) => {
		...
		const compProxy = {
			...
			setAriaSort: (sort) => {
				if (eGui.current) {
					if (sort) _setAriaSort(eGui.current, sort);
...
var HeaderFilterCellComp = ({ ctrl }) => {
	...
	(0, import_react.useEffect)(() => {
		if (userCompDetails?.componentFromFramework) {
			...
			} else warnReactiveCustomComponents(context.getId());
		}
	}, [userCompDetails]);
...
var HeaderRowComp = ({ ctrl, setGuiRef }) => {
	...
	const [tabIndex, setTabIndex] = (0, import_react.useState)(() => _isHeaderFocusSuppressed(beans) ? void 0 : gos.get("tabIndex"));
...
var GridHeaderComp = ({ eGridViewport }) => {
	...
	const setRef2 = (0, import_react.useCallback)((eRef) => {
		...
		const compProxy = { toggleCss: (name, on) => cssManager.current.toggleCss(name, on) };
...
var useJsCellRenderer = (showDetails, showTools, eCellValue, cellValueVersion, jsCellRendererRef, eGui, suppressInlineEditRenderer = false, onRendererDestroyed) => {
	const { context } = (0, import_react.useContext)(BeansContext);
	const onRendererDestroyedRef = (0, import_react.useRef)(onRendererDestroyed);
	onRendererDestroyedRef.current = onRendererDestroyed;
	const destroyCellRenderer = (0, import_react.useCallback)((resetTooltip = true) => {
...
var CellComp = ({ cellCtrl, printLayout, editingCell }) => {
	...
	const setCellEditorRef = (0, import_react.useCallback)((cellEditor) => {
		...
		if (cellEditor) {
			...
			setTimeout(() => {
				if (!cellCtrl.isAlive() || context.isDestroyed()) return;
	...
	const resetCellRendererTooltip = (0, import_react.useCallback)(() => cellCtrl.resetCellRendererTooltip(), [cellCtrl]);
	...
	(0, import_react.useLayoutEffect)(() => {
		...
		if (cellRendererRef.current.refresh(newCompDetails.params) != true) {
				...
				const setDetails = () => {
					setRenderDetails((prev) => {
						if (prev?.compDetails !== compDetails || prev?.value !== value || prev?.force !== force) {
							...
							if (rendererRemoved || rendererReplaced) cellCtrl.resetCellRendererTooltip();
							return {
								value,
								compDetails,
								force
			...
			setEditDetails: (compDetails, popup, popupPosition, reactiveCustomComponents) => {
				if (compDetails) {
					...
					if (compDetails.componentFromFramework) {
						if (reactiveCustomComponents) compProxy2 = new CellEditorComponentProxy(compDetails.params, () => setRenderKey((prev) => prev + 1));
						else warnReactiveCustomComponents(beans.context.getId());
					}
					setEditDetails({
						compDetails,
						popup,
...
var GridBodyComp = () => {
	...
	const [pinnedColumnsOverflowing, setPinnedColumnsOverflowing] = (0, import_react.useState)(false);
	...
	const gridViewportClasses = (0, import_react.useMemo)(() => classesList("ag-grid-viewport", layoutClass, pinnedColumnsOverflowing ? "ag-pinned-columns-overflow" : null), [layoutClass, pinnedColumnsOverflowing]);
	...
	const gridViewportStyle = (0, import_react.useMemo)(() => ({
	...
	return /* @__PURE__ */ import_react.createElement("div", {
		...
		role: "presentation",
		style: topStyle
	}, topElement && gridViewportElement && /* @__PURE__ */ import_react.createElement(gridHeaderComp_default, { eGridViewport: gridViewportElement }), /* @__PURE__ */ import_react.createElement("div", {
		ref: eTopExtraRows,
...
var TabGuardCompRef = (props, forwardRef4) => {
	...
	(0, import_react.useImperativeHandle)(forwardRef4, () => ({
...
var TabGuardComp = (0, import_react.forwardRef)(TabGuardCompRef);
var tabGuardComp_default = (0, import_react.memo)(TabGuardComp);
var GridComp = ({ context }) => {
	...
	const ariaDescriptionRef = (0, import_react.useRef)(null);
	...
	const setRef2 = (0, import_react.useCallback)((eRef) => {
		...
		const compProxy = {
			...
			focusNextElementOutsideContainer: (up, eExcludeContainers) => {
				...
				return eRootWrapper ? tabGuardRef.current?.focusNextElementOutsideContainer(up, [eRootWrapper, ...eExcludeContainers]) ?? false : false;
			},
			updateLayoutClasses: setLayoutClass,
	...
	for (const propKey of Object.keys(nextProps)) {
		if (excludeReactCompProps.has(propKey)) {
			if (deprecatedReactCompProps.has(propKey)) {
				if (gridId) _warnForGrid(gridId, 274, { prop: propKey });
				else _warnWithoutAttribution(274, { prop: propKey });
			}
			continue;
		}
		...
		if (_getGridOption(gridOptions, "reactiveCustomComponents")) {
			const getComponentClass = (propertyName) => {
				switch (propertyName) {
					...
					case "columnLabelRenderer": return ColumnSelectionLabelRendererComponentWrapper;
			...
			case "menuItem":
			case "cellRenderer":
			case "columnLabelRenderer": warnReactiveCustomComponents(this.gridId);
		}
...
var DetailCellRenderer = (0, import_react.forwardRef)((props, ref) => {
	...
	if (props.template) beans.log.warn(230);
	const setRef2 = (0, import_react.useCallback)((eRef) => {
