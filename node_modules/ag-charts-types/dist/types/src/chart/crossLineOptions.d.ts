...
export interface AgLineCrossLineOptions<TValue = AxisValue, LabelType = AgBaseCrossLineLabelOptions, TContext = ContextDefault> extends AgCommonCrossLineOptions<LabelType, TContext> {
    /** Renders the Cross Line as a single line positioned at `value`. */
    type: 'line';
    /** The data value at which the line should be positioned. */
    value: TValue;
}
export interface AgRangeCrossLineOptions<TValue = AxisValue, LabelType = AgBaseCrossLineLabelOptions, TContext = ContextDefault> extends AgCommonCrossLineOptions<LabelType, TContext> {
...
export type AgBaseCrossLineOptions<TValue = AxisValue, LabelType = AgBaseCrossLineLabelOptions, TContext = ContextDefault> = AgLineCrossLineOptions<TValue, LabelType, TContext> | AgRangeCrossLineOptions<TValue, LabelType, TContext>;
export interface AgCrossLineThemeOptions<LabelType = AgBaseCrossLineLabelOptions> extends Omit<AgCommonCrossLineOptions<LabelType, ContextDefault>, 'id' | 'listeners'> {
    /** The colour to use for the fill of the range. */
    fill?: CssColor;
    /** The opacity of the fill for the range. */
    fillOpacity?: Opacity;
