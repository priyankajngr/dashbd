...
type DatumKeyTraversable<T> = T extends ReadonlyArray<any> | Date | ((...args: any[]) => any) ? false : T extends object ? true : false;
type DatumKeyPaths<T, Depth extends number> = [Depth] extends [never] ? never : {
    [K in keyof T & string]: DatumKeyTraversable<NonNullable<T[K]>> extends true ? K | `${K}.${DatumKeyPaths<NonNullable<T[K]>, DatumKeyDepth[Depth]>}` : K;
}[keyof T & string];
