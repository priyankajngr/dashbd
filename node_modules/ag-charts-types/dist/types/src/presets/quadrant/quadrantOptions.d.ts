...
import type { AgCartesianAxisCrossAtPlacement, AgNumberAxisOptions, AgSeriesAreaBackgroundRegion, AgSeriesAreaBackgroundRegionLabel } from '../../chart/cartesianOptions';
import type { AgNumericValue } from '../../chart/dataValues';
import type { AgErrorBarOptions } from '../../chart/errorBarOptions';
import type { AgChartLabelFormatterParams, AgChartLabelStyleOptions, AgChartLabelStylerParams } from '../../chart/labelOptions';
import type { AgSeriesTooltip } from '../../chart/tooltipOptions';
import type { DatumKey, PixelSize } from '../../chart/types';
import type { AgScatterSeriesItemStylerParams, AgScatterSeriesLabel, AgScatterSeriesLabelFormatterParams, AgScatterSeriesOptionsKeys, AgScatterSeriesOptionsNames, AgScatterSeriesThemeableOptions, AgScatterSeriesTooltipRendererParams } from '../../series/cartesian/scatterOptions';
...
export interface AgQuadrantPreset<TDatum, TContext> extends Omit<AgScatterSeriesOptionsKeys<TDatum>, 'colorKey'>, Omit<AgScatterSeriesOptionsNames, 'colorName' | 'legendItemName'>, Omit<AgScatterSeriesThemeableOptions<TDatum, TContext>, 'colorScale' | 'errorBar' | 'itemStyler' | 'label' | 'showInLegend' | 'showInMiniChart' | 'title' | 'tooltip'> {
...
export interface AgQuadrantRegionsLabelOptions extends Omit<AgSeriesAreaBackgroundRegionLabel, 'position' | 'text' | 'xOffset' | 'yOffset'> {
 ...
 */
export type AgQuadrantRegionLabelPosition = 'outside-outer' | 'outside-center' | 'outside-inner' | 'inside-outer-outer' | 'inside-outer-center' | 'inside-outer-inner' | 'inside-center-outer' | 'inside-center' | 'inside-center-inner' | 'inside-inner-outer' | 'inside-inner-center' | 'inside-inner-inner';
...
export interface AgQuadrantAxisOptions<TContext> extends Omit<AgNumberAxisOptions<TContext>, 'crossAt' | 'crossLines' | 'keys' | 'reverse' | 'position' | 'type'> {
}
export interface AgQuadrantItemStylerParams<TDatum, TContext> extends AgScatterSeriesItemStylerParams<TDatum, TContext> {
    /** The region the marker falls in, determined by comparing its x- and y-values against the pivot. */
    region: AgQuadrantRegion;
}
export interface AgQuadrantTooltipRendererParams<TDatum, TContext> extends AgScatterSeriesTooltipRendererParams<TDatum, TContext> {
    /** The region the marker falls in, determined by comparing its x- and y-values against the pivot. */
    region: AgQuadrantRegion;
    /** The human-readable description of the size values, as supplied via `sizeName`, present when `sizeKey` is set. */
    sizeName?: string;
}
export interface AgQuadrantLabelOptions<TDatum, TContext> extends Omit<AgScatterSeriesLabel<TDatum, TContext>, 'formatter' | 'itemStyler'> {
    ...
    itemStyler?: Styler<AgChartLabelStylerParams<TDatum, TContext> & AgQuadrantLabelFormatterParams<TDatum>, AgChartLabelStyleOptions>;
}
