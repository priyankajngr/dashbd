...
/** @knipIgnore Used in tests */
export type NestedPath<TValue, Prefix extends string, TValueNestedChild, TDepth extends any[]> = TValue extends object ? `${Prefix}.${TDepth['length'] extends 5 ? any : NestedFieldPaths<TValue, TValueNestedChild, TDepth>}` : never;
/**
...
type NestedOnlyPaths<TData, TValue, TDepth extends any[]> = {
    [TKey in StringOrNumKeys<TData>]: TData[TKey] extends ((...args: any[]) => any) | undefined ? never : TData[TKey] extends any[] | undefined ? `${TKey}.${number}` : NestedPath<TData[TKey], `${TKey}`, TValue, [...TDepth, any]>;
}[StringOrNumKeys<TData>];
/**
 * Returns a union of all possible paths to nested fields in `TData`.
 */
export type NestedFieldPaths<TData = any, TValue = any, TDepth extends any[] = []> = OwnFieldPaths<TData, TValue> | NestedOnlyPaths<TData, TValue, TDepth>;
...
export type TooltipComponentSelectorFunc<TData = any, TValue = any, TContext = any> = (params: ITooltipParams<TData, TValue, TContext>) => TooltipComponentSelectorResult | undefined;
