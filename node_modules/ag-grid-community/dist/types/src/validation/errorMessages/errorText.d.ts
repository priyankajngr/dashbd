...
export declare const AG_GRID_ERRORS: {
 ...
 To run logic when the grid is about to be destroyed use the \`gridPreDestroyed\` event. See: ${string}`;
    27: ({ fnName, module }: {
        fnName: string;
        module: string;
    ...
    36: ({ type }: {
        type: string;
    }) => `\`colDef.type\` \`${string}\` does not correspond to defined \`gridOptions.columnTypes\``;
...
${string}`;
  ...
  - \`colDef.cellDataType = 'object'\``;
    ...
    72: ({ keys }: {
        keys: string[];
    }) => readonly ["ignoring `FilterOptionDef` as it is missing ", string[]];
    ...
    79: () => "`filterParams.maxNumConditions` must be greater than or equal to one.";
    80: () => "`filterParams.numAlwaysVisibleConditions` must be greater than or equal to one.";
    ...
    99: () => "Since v32, `api.hideOverlay()` does not hide the loading UI when the `loading` grid option is set. Set `loading=false` instead.";
    101: ({ propertyName, componentName, suggestions, }: {
        propertyName: string;
...
Please turn off ${string} to enable Excel table exports.`;
    ...
    200: ({ reasonOrId, moduleName, reports, gridScoped, gridId, rowModelType, additionalText, isUmd, usesAgGridProvider, }: {
        reasonOrId: string | keyof MissingModuleErrors;
        moduleName: ValidationModuleName | ValidationModuleName[];
    ...
    251: ({ chartType }: {
        chartType: string;
    }) => `Unable to create chart as an invalid chartType = \`${string}\` was supplied.`;
    ...
    306: ({ version, name, message }: {
        version: string;
        name: string;
        message?: string;
    }) => string;
    307: ({ objectName, name, suggestions, hasContext, }: {
        objectName: string;
        name: string;
        suggestions: string[];
        hasContext?: boolean;
    }) => string;
    308: ({ version, apiMethod, replacement, message, }: {
        version: string;
        apiMethod: string;
        replacement?: string;
        message?: string;
    }) => string;
    309: ({ name, rowModel, supportedRowModels }: {
        name: string;
        rowModel: string;
        supportedRowModels?: string[];
    }) => string;
    310: ({ objectName, url }: {
        objectName: string;
        url: string;
    }) => string;
    311: ({ functionName, rowModels }: {
        functionName: string;
        rowModels?: string[];
    }) => string;
    312: ({ chartType, renamedChartType }: {
        chartType: string;
        renamedChartType: string;
    }) => string;
    313: ({ paramsType, property }: {
        paramsType?: string;
        property: string;
    }) => string;
    314: ({ key, expectedType, actualType, value, }: {
        key: string;
        expectedType: string;
        actualType: string;
        value?: unknown;
    }) => string;
    315: ({ key, failedKey, required, reason, }: {
        key: string;
        failedKey: string;
        required?: string[];
        reason?: string;
    }) => string;
    316: ({ property, value, expectedType }: {
        property: string;
        value?: unknown;
        expectedType: string;
    }) => string;
    ...
    318: ({ feature, conflictsWith, advice }: {
        feature: string;
        conflictsWith: string;
        advice?: string;
    }) => string;
    319: ({ feature, requirement }: {
        feature: string;
        requirement: string;
    }) => string;
    320: ({ property, allowed, value }: {
        property: string;
        allowed?: string[];
        value?: unknown;
    }) => string;
    321: ({ property, expected }: {
        property: string;
        expected: string;
    }) => string;
    322: ({ message }: {
    ...
    }) => `ignoring \`defaultOption\` \`${string}\` as it is not one of the filter's \`filterOptions\``;
    327: ({ pattern }: {
        pattern: string;
    }) => `ignoring \`filterParams.allowedCharPattern\` \`${string}\` as it does not compile to a character pattern`;
    328: ({ rowModel }: {
        rowModel: string;
    }) => `continuous \`fitCellContents\` auto-sizing is not supported by the '${string}' row model, so it has been ignored`;
    329: ({ error }: {
        error: unknown;
    }) => unknown[];
    330: ({ fontFamily, registeredFamilies }: {
        fontFamily: string;
        registeredFamilies: string[];
    }) => string;
    331: ({ rowId }: {
        rowId: string;
    }) => `Row ID \`${string}\` is reserved by AG Grid and should not be returned from \`getRowId\`.`;
    332: ({ replacements }: {
        replacements: string[];
    }) => `Legacy theme CSS variables are set on the grid, but the Theming API is in use and ignores them, so they have no effect: ${string}. Either set the equivalent Theming API variable, or pass the string \`legacy\` to the \`theme\` grid option to keep using v32 style themes. See the migration guide: ${string}/theming-migration/`;
    333: ({ error }: {
        error: unknown;
    }) => unknown[];
};
 ...
 */
export declare const MAX_ERROR_ID = 333;
...
export type LogArgs<TId extends ErrorId> = GetErrorParams<TId> extends undefined ? [id: TId] : [id: TId, params: GetErrorParams<TId>];
...
declare const MISSING_MODULE_REASONS: {
    ...
    readonly 4: "the `isAnyOf` / `isNoneOf` filter options in the Advanced Filter";
};
